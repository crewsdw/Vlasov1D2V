...
class Stepper:
    ...
    def main_loop(self, distribution, basis, elliptic, grids, dg_flux, refs, save_file):
        ...
        t0 = timer.time()
        f_initial = cp.copy(distribution.arr)
        # Initial field energy
        field_energy = np.array([elliptic.electric_energy(grid=grids.x).get()])
        time_array = np.array([self.time])
        # Look at ghost cells
        print('\nInitializing time-step...')
        # Adapt time-step
        max_speeds = np.absolute(np.array([grids.u.arr_max,
                                           ...
                                           refs.electron_acceleration_multiplier * abs(elliptic.magnetic_field *
                                                                                       grids.u.arr_max)]))
        self.adapt_time_step(max_speeds=max_speeds, dx=grids.x.dx, du=grids.u.dx, dv=grids.v.dx)
        while self.time < self.final_time:
            # print('Step is ' + str(self.steps_counter))
            # Perform RK update
            ...
            time_array = np.append(time_array, self.time)
            field_energy = np.append(field_energy, elliptic.electric_energy(grid=grids.x).get())
            ...
            if cp.isnan(distribution.arr).any():
                ...
                quit()
            if self.time > 150:  # self.steps_counter == 30000:
                ...
                # vidx_x = 3
                # vidx_u =
                # dfdt = dg_flux.semi_discrete_rhs(function=distribution.arr,
                #                                  elliptic=elliptic,
                #                                  basis=basis,
                #                                  grids=grids)
                ...
                df = distribution.arr  # - f_initial  # f_next - f_initial  #
                f0f = cp.asnumpy(df[1:-1, :, 1:-1, :, 1:-1, :].reshape(
                    (grids.x.res * grids.x.order, grids.u.res * grids.u.order, grids.v.res * grids.v.order)))
                ...
                # cb3 = np.linspace(0.05, 1, num=50)
                # contours = grid.contour(cb2)
                # p.add_mesh(contours.outline_corners())
                # p.add_mesh(contours, opacity=0.75)
                # p.show()
                ...
                # Print lower corner of every element
                # corners = cp.zeros_like(dfdt)
                # corners[:, :, :, 0, :, :] = -1
                # corners[:, :, :, -1, :, :] = +1
                # corners[:, :, :, :, :, 0] = -1
                # corners[:, :, :, :, :, -1] = +1
                ...
                # plt.figure()
                # plt.imshow(corners_f[30, 85:107, 85:107])
                # plt.colorbar()

                # plt.figure()
                # plt.imshow(f0f[30, 85:107, 85:107])
                # plt.colorbar()
                #
                # print(np.amin(f0f[11, 85:107, 85:107]))
                # print(np.amax(f0f[11, 85:107, 85:107]))
                #
                # l = 0  # 121 - 20
                # h = -1  # 121 + 20
    ...
    def ssp_rk_update(self, distribution, basis, grids, elliptic, dg_flux, refs):
        ...
        # f.arr_next[grids.no_ghost_slice] = self.coefficients[0] * f.arr_explicit_copy[grids.no_ghost_slice]
        # new_arr = self.coefficients[0]
        # Time-step loop
        for i in range(1, self.time_order):
            ...
            # df_dt = dg_flux.semi_discrete_rhs(function=distribution.arr_explicit_copy,
            #                                   elliptic=elliptic,
            #                                   basis=basis,
            #                                   grids=grids)[grids.no_ghost_slice]
            # distribution.arr_explicit_copy[grids.no_ghost_slice] = cp.add(distribution.arr_explicit_copy
            #                                                               [grids.no_ghost_slice],
            #                                                    self.dt * df_dt)
            ...
            # f.arr_next[grids.no_ghost_slice] += self.coefficients[i] * f.arr_explicit_copy[grids.no_ghost_slice]
            # f.arr_next[grids.no_ghost_slice] = cp.add(self.coefficients[i] *
            # f.arr_explicit_copy[grids.no_ghost_slice],
            #                                           f.arr_next[grids.no_ghost_slice])
        ...
        max_speeds = np.absolute(np.array([grids.u.arr_max,
                                           ...
                                           refs.electron_acceleration_multiplier * abs(elliptic.magnetic_field *
                                                                                       grids.u.arr_max)]))
        self.adapt_time_step(max_speeds=max_speeds, dx=grids.x.dx, du=grids.u.dx, dv=grids.v.dx)
        # df_dt[abs(df_dt) < 1.0e-15] = 0
        # f.arr_next[grids.no_ghost_slice] += self.coefficients[self.time_order - 1] * self.dt * df_dt
        # f.arr_next[grids.no_ghost_slice] = cp.add(self.coefficients[self.time_order - 1] * self.dt * df_dt,
        #                                           f.arr_next[grids.no_ghost_slice])
        # f.arr_next[grids.no_ghost_slice] = f.arr_next[grids.no_ghost_slice] / cp.sum(self.coefficients)

    def adapt_time_step(self, max_speeds, dx, du, dv):
        # self.courant = 0.055 / np.sqrt(3)
        self.dt = self.courant / ((max_speeds[0] / dx) + (max_speeds[1] / du) + (max_speeds[2] / dv))  # / 1.25
